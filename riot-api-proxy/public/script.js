...
let svgBar, svgLine, svgArea, svgScatter, svgHeatmap, svgDualBar, svgRadar, svgTree;
...
function updateDualBarChart(data) {
  ...
  if (selectedMatch == null) {
    ...
    for (let [key, value] of playerTotals) {
      playerTotals.set(key, value / count);
    }
...
function updateTreeChart(data) {
  ...
  //cell.each generated by chatgpt
  cell.each(function (d) {
    ...
    while (textWidth > rectWidth && truncatedLabel.length > 0) {
      truncatedLabel = truncatedLabel.slice(0, -1); // Remove one character
      tempText.text(truncatedLabel + ellipsis); // Add ellipsis for measurement
